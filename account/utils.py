...
def report_for_an_account(account):
	...
	from_acc_temp_lst = list(qs.values_list("date_created", "id", "amount"))
	temp_lst = []
	...
	to_acc_temp_lst = list(qs.values_list("date_created", "id", "amount"))
	temp_lst = []
	...
	expense_temp_lst = list(Expense.objects.filter(account=account).values_list("date_created", "id", "cost"))	
